...
func TestCompareTestResults_NilPointerPanic(t *testing.T) {
	...
	afterResults := []testJSON{
		{
			Time:    time.Now(),
			Package: "github.com/spinility/sketch-neomxm/newpkg",
			Action:  "run",
...
func TestCompareTestResults_NilPointerPanic_FailingTest(t *testing.T) {
	...
	if len(regressions) > 0 {
		...
		if regression.Package != "github.com/spinility/sketch-neomxm/newpkg" {
			t.Errorf("Expected package 'github.com/spinility/sketch-neomxm/newpkg', got %q", regression.Package)
		}
...
func TestCompareTestResults_ExistingPackage(t *testing.T) {
	...
	beforeResults := []testJSON{
		{
			Time:    time.Now(),
			Package: "github.com/spinility/sketch-neomxm/existing",
			Action:  "pass",
			Test:    "TestExisting",
			Elapsed: 0.001,
		},
