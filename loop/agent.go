...
import (
	...
	"github.com/spinility/sketch-neomxm/claudetool/browse"
	"github.com/spinility/sketch-neomxm/claudetool/codereview"
	"github.com/spinility/sketch-neomxm/claudetool/onstart"
	"github.com/spinility/sketch-neomxm/cortex"
	"github.com/spinility/sketch-neomxm/experiment"
	"github.com/spinility/sketch-neomxm/llm"
	"github.com/spinility/sketch-neomxm/llm/ant"
	"github.com/spinility/sketch-neomxm/llm/conversation"
	"github.com/spinility/sketch-neomxm/mcp"
	"github.com/spinility/sketch-neomxm/skabandclient"
...
type Agent struct {
	...
	// Cortex for intelligent expert routing
...
func NewAgent(config AgentConfig) *Agent {
	...
	// Initialize cortex with default config
	cortexConfig := cortex.DefaultConfig()
	cortexInstance, err := cortex.NewCortex(cortexConfig, config.Service)
	if err != nil {
		slog.WarnContext(config.Context, "Failed to initialize cortex, proceeding without it", "error", err)
		agent.cortex = nil
	} else {
		slog.InfoContext(config.Context, "Cortex initialized successfully")
		agent.cortex = cortexInstance
	}
...
func (a *Agent) processUserMessage(ctx context.Context) (*llm.Response, error) {
	...
	// Optionally route through cortex to select expert
	if a.cortex != nil {
		slog.InfoContext(ctx, "Routing request through cortex for expert selection")
		request := a.buildLLMRequest(userMessage)
		choice, err := a.cortex.ChooseExpert(ctx, request)
		if err != nil {
			slog.WarnContext(ctx, "Cortex expert selection failed, proceeding without cortex", "error", err)
		} else if choice != nil {
			slog.InfoContext(ctx, "Cortex selected expert",
				"expert", choice.Expert.Profile.Name,
				"model", choice.Expert.Profile.Model,
				"reasoning", choice.Reasoning,
			)
			// Modify system prompt temporarily for this request
			if convo, ok := a.convo.(*conversation.Convo); ok {
				originalPrompt := convo.SystemPrompt
				// Prepend expert's system prompt
				convo.SystemPrompt = choice.SystemPrompt + "\n\n" + originalPrompt
				defer func() {
					convo.SystemPrompt = originalPrompt
				}()
			}
		}
	}
...
// buildLLMRequest builds an llm.Request from the current conversation state
// This is needed when routing through the cortex
func (a *Agent) buildLLMRequest(userMessage llm.Message) *llm.Request {
